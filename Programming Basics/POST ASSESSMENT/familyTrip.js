function main(budget, numOfNights, pricePerNight, extraCostPercentage) {
	...
	if (tripCost <= budget) {
		...
		console.log(
			`The Smiths will be left with ${moneyLeft.toFixed(
				2
			)} dollars after vacation.`
		);
	} else {
 ...
 *  that calculates whether their *budget(input) will be enough for their trip.
 * Knowing, how many nights(input) they have planned for their trip, the price
 * per night(input) and the percent of the budget(input) that has been provided for additional costs.
 * Keep in mind that if the number of nights is greater than 7,
 * the price per night is reduced by 5% (bonusDiscount).
 */
